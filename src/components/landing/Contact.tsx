    ...
    <section id="contact" className="py-20 bg-gray-50">
      <div className="landing-container max-w-3xl">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-10">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">{t('heading')}</h2>
          <p className="text-xl text-gray-600">{t('subtitle')}</p>
        </motion.div>

        {sent ? (
          <div className="rounded-2xl border border-green-200 bg-green-50 p-6 text-center text-green-700">{t('sentMessage')}</div>
        ) : (
          <form onSubmit={submit} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={t('form.name')} className="h-11 rounded-xl border border-gray-200 px-4 text-sm" />
              <input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder={t('form.email')} className="h-11 rounded-xl border border-gray-200 px-4 text-sm" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder={t('form.phone')} className="h-11 rounded-xl border border-gray-200 px-4 text-sm" />
              <input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder={t('form.subject')} className="h-11 rounded-xl border border-gray-200 px-4 text-sm" />
            </div>
            <textarea required value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder={t('form.message')} rows={4} className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" />
