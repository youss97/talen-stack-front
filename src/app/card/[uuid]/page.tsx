...
export default function PublicCardPage() {
  ...
  if (error || !card) return <div className="min-h-screen flex items-center justify-center text-red-500">Carte introuvable</div>;

  const brand = card.brand_color || "#8AB925";
  const template = card.branding?.template || "snow";
  const t = getTheme(template, brand);
  ...
  const btnStyle = { background: t.btnBg, color: t.btnText, border: `1px solid ${t.btnBorder}` };
  ...
  return (
    <div className="min-h-screen flex justify-center" style={{ background: t.page }}>
      <div className="w-full max-w-md px-6 pt-12 pb-16 flex flex-col items-center">
        ...
        {/* Identité */}
        <h1 className="mt-4 text-xl font-bold text-center" style={{ color: t.text }}>{card.full_name}</h1>
        {card.position && <p className="mt-0.5 text-sm text-center" style={{ color: t.sub }}>{card.position}</p>}
        ...
        {(card.company_logo || card.company_label || card.company_name) && (
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-sm font-medium" style={{ color: t.text }}>{card.company_label || card.company_name}</span>
          </div>
        )}
        ...
        {texts.length > 0 && (
          <div className="mt-3 text-center space-y-0.5">
            {texts.map((tx, i) => (
              <p key={i} className="text-sm" style={{ color: t.sub }}>
                {tx.label ? <span className="font-medium" style={{ color: t.text }}>{tx.label} : </span> : null}{tx.value}
              </p>
            ))}
          </div>
        )}
        ...
        <div className="mt-7 w-full space-y-3">
          {card.phone && <LinkButton href={`tel:${card.phone}`} label="Appeler" />}
          {card.email && <LinkButton href={`mailto:${card.email}`} label="Envoyer un email" />}
          {socials.map(([k, v]) => <LinkButton key={k} href={v} label={k.charAt(0).toUpperCase() + k.slice(1)} />)}
          {links.map((l, i) => <LinkButton key={`l${i}`} href={l.value} label={l.label || "Lien"} />)}
          <LinkButton href={`${API}/public/card/${uuid}/vcard`} label="Ajouter aux contacts" />
        </div>

        <p className="mt-10 text-xs" style={{ color: t.sub }}>Powered by TalentStack</p>
      </div>
    </div>
  );
}
