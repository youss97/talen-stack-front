...
export default function PublicSiteSettingsPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      addToast("success", "Enregistré", "Configuration du site public mise à jour");
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <button onClick={() => router.push("/settings")} className="mb-4 inline-flex items-center gap-1.5 text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400">
        ← Retour aux paramètres
      </button>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-800 dark:text-white/90">Site public & branding</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Personnalisez la page publique de votre entreprise (offres, candidature spontanée).</p>
      </div>

      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6 space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Slug public (URL)</label>
          <input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-") })} placeholder="mon-entreprise" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm dark:bg-gray-900 dark:border-gray-700" />
          {publicUrl && (
            <p className="mt-1.5 text-xs text-gray-500">
              Lien : <a href={publicUrl} target="_blank" rel="noopener noreferrer" className="text-brand-600 underline">{publicUrl}</a>
            </p>
          )}
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Présentation</label>
          <textarea value={form.public_description} onChange={(e) => setForm({ ...form, public_description: e.target.value })} rows={4} placeholder="Présentez votre entreprise aux candidats…" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:bg-gray-900 dark:border-gray-700" />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Couleur de marque</label>
