...
const TEMPLATES = [
  { id: "snow", label: "Clair (Snow)" },
  { id: "dark", label: "Sombre" },
  { id: "brand", label: "Couleur de marque" },
  { id: "gradient", label: "Dégradé" },
  { id: "mint", label: "Pastel" },
];
...
export default function BusinessCardsPage() {
  ...
  const downloadQR = () => {
    ...
    a.href = cardImageUrl;
    a.download = `carte-${(qrCard?.full_name || "visite").replace(/\s+/g, "_")}.png`;
    a.click();
  };
  const printQR = () => {
    if (!cardImageUrl) return;
    ...
    if (!w) return;
    w.document.write(`<html><head><title>Carte ${qrCard?.full_name || ""}</title></head><body style="margin:0;display:flex;align-items:center;justify-content:center;height:100vh"><img src="${cardImageUrl}" style="max-width:100%"/><script>window.onload=function(){window.print()}</script></body></html>`);
    w.document.close();
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      addToast("success", "Succès", editing ? "Carte modifiée" : "Carte créée");
      setOpen(false);
    } catch (err) {
      addToast("error", "Erreur", getApiErrorMessage(err, "Erreur lors de l'enregistrement"));
    }
  };
  ...
  const remove = async (c: BusinessCard) => {
    if (!confirm(`Supprimer la carte de ${c.full_name} ?`)) return;
    try { await deleteCard(c.id).unwrap(); addToast("success", "Succès", "Carte supprimée"); } catch { /* */ }
  };
  const copyLink = (c: BusinessCard) => {
    navigator.clipboard.writeText(`${window.location.origin}/card/${c.id}`);
    addToast("success", "Lien copié", "Lien de la carte copié");
  };
    ...
    <div className="p-6">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="flex flex-wrap items-center justify-between gap-3 p-5 border-b border-gray-100 dark:border-gray-800">
          <div>
            <h1 className="text-lg font-semibold text-gray-800 dark:text-white">Cartes de visite</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Cartes de visite QR partageables (RH, Client, Employé)</p>
          </div>
          <Button onClick={openMyCard}>Créer ma carte</Button>
        </div>
              ...
              {(cards || []).map((c) => (
                <div key={c.id} className="rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{c.owner_type}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${c.is_active ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-500"}`}>{c.is_active ? "Active" : "Inactive"}</span>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-2">
                    <button onClick={() => { setCardImageUrl(""); setQrCard(c); }} className="col-span-3 rounded-lg bg-brand-500 px-3 py-2 text-xs font-semibold text-white hover:bg-brand-600 transition-colors">QR & Carte</button>
                    <a href={`/card/${c.id}`} target="_blank" rel="noopener noreferrer" className="rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2 text-center text-xs font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 transition-colors">Aperçu</a>
                    <button onClick={() => copyLink(c)} className="rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2 text-xs font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 transition-colors">Lien</button>
                    <button onClick={() => openEdit(c)} className="rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2 text-xs font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 transition-colors">Modifier</button>
                    <button onClick={() => toggleActive(c)} className="col-span-2 rounded-lg px-3 py-2 text-xs font-medium transition-colors bg-amber-50 text-amber-700 hover:bg-amber-100 dark:bg-amber-900/20 dark:text-amber-300">{c.is_active ? "Désactiver" : "Activer"}</button>
                    <button onClick={() => remove(c)} className="rounded-lg px-3 py-2 text-xs font-medium bg-red-50 text-red-600 hover:bg-red-100 transition-colors dark:bg-red-900/20 dark:text-red-300">Supprimer</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} className="max-w-lg">
        <div className="p-6 max-h-[85vh] overflow-y-auto">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">{editing ? "Modifier ma carte" : "Ma carte de visite"}</h2>
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-400">Photo issue de votre profil</span>
            </div>
            ...
            <input value={form.full_name || ""} onChange={(e) => setForm({ ...form, full_name: e.target.value })} placeholder="Nom complet *" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm dark:bg-gray-900 dark:border-gray-700" />
            <input value={form.position || ""} onChange={(e) => setForm({ ...form, position: e.target.value })} placeholder="Fonction / Poste" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm dark:bg-gray-900 dark:border-gray-700" />
            <input value={form.company_label || ""} onChange={(e) => setForm({ ...form, company_label: e.target.value })} placeholder="Société" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm dark:bg-gray-900 dark:border-gray-700" />
            <input value={form.email || ""} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Email" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm dark:bg-gray-900 dark:border-gray-700" />
            <input value={form.phone || ""} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="Téléphone" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm dark:bg-gray-900 dark:border-gray-700" />
            <input value={form.linkedin || ""} onChange={(e) => setForm({ ...form, linkedin: e.target.value })} placeholder="LinkedIn (URL)" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm dark:bg-gray-900 dark:border-gray-700" />
            <input value={form.website || ""} onChange={(e) => setForm({ ...form, website: e.target.value })} placeholder="Site web (URL)" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm dark:bg-gray-900 dark:border-gray-700" />

            {/* Champs personnalisés */}
            <div className="pt-2">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Champs personnalisés (liens / texte)</span>
                <button type="button" onClick={addCustom} className="text-xs px-2 py-1 rounded border border-gray-200 dark:border-gray-700">+ Ajouter</button>
              </div>
              <div className="space-y-2">
                {(form.custom_fields || []).map((cf, i) => (
                  <div key={i} className="flex gap-2">
                    ...
                    <input value={cf.label} onChange={(e) => setCustom(i, { label: e.target.value })} placeholder="Libellé" className="h-9 flex-1 rounded-lg border border-gray-300 px-2 text-xs dark:bg-gray-900 dark:border-gray-700" />
                    <input value={cf.value} onChange={(e) => setCustom(i, { value: e.target.value })} placeholder={cf.type === "link" ? "URL" : "Valeur"} className="h-9 flex-1 rounded-lg border border-gray-300 px-2 text-xs dark:bg-gray-900 dark:border-gray-700" />
          ...
          <div className="flex justify-end gap-3 mt-5">
            ...
            <Button onClick={submit} disabled={creating || updating || !form.full_name}>{creating || updating ? "..." : "Enregistrer"}</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={!!qrCard} onClose={() => setQrCard(null)} className="max-w-xl">
        <div className="p-6 text-center">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-white">Carte de visite à partager</h2>
          <p className="text-sm text-gray-500 mb-4">{qrCard?.full_name}</p>
          <div className="flex justify-center min-h-[180px] items-center">
            {cardImageUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={cardImageUrl} alt="Carte de visite" className="w-full max-w-md rounded-xl border border-gray-100 shadow-sm" />
            ) : (
              <div className="w-8 h-8 border-4 border-gray-200 border-t-brand-500 rounded-full animate-spin" />
            )}
          </div>
          <p className="mt-3 text-xs text-gray-400">Un scan du QR ouvre la carte interactive.</p>
          <div className="mt-5 flex justify-center gap-3">
            <Button variant="outline" onClick={printQR} disabled={!cardImageUrl}>Imprimer</Button>
            <Button onClick={downloadQR} disabled={!cardImageUrl}>Télécharger (PNG)</Button>
          </div>
        </div>
      </Modal>
    </div>
