...
export default function RolesPage() {
  ...
  const columns: Column<Role>[] = [
    ...
    {
      key: "users_count",
      header: t("list.columns.usersCount"),
      render: (value) => {
  ...
  const handleEditClick = async (role: Role) => {
    if (role.is_protected) {
      addToast("warning", t("toasts.unauthorizedTitle"), t("toasts.unauthorizedEditMessage"));
      return;
    }
    try {
      const result = await getRoleById(role.id).unwrap();
  ...
  const handleDeleteClick = (role: Role) => {
    if (role.is_protected) {
      addToast("warning", t("toasts.unauthorizedTitle"), t("toasts.unauthorizedDeleteMessage"));
      return;
    }
    if ((role.users_count ?? 0) > 0) {
      addToast(
        "warning",
        t("toasts.cannotDeleteTitle"),
        t("toasts.cannotDeleteMessage", { count: role.users_count ?? 0 }),
      );
      return;
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      addToast("error", tc("status.error"), getErrorMessage(error, t("toasts.deleteErrorDefault")));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleFormSubmit = async (data: CreateRoleFormData, actionIds: string[]) => {
    ...
    try {
      ...
      const defaultMsg = selectedRole
        ? t("toasts.updateErrorDefault")
        : t("toasts.createErrorDefault");
      ...
      addToast("error", tc("status.error"), errorMessage);
  ...
  return (
    <div>
      ...
      <ConfirmModal
        ...
        message={t("confirmDelete.message", { name: confirmModal.role?.name ?? "" })}
