...
export default function RecruitmentPage() {
  ...
  const handleExportExcel = () => {
    if (!data?.data || data.data.length === 0) {
      addToast("warning", t("toast.noDataTitle"), t("toast.noDataMessage"));
      return;
    }
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : t("toast.exportErrorDefault");
  ...
  const handleAssignRequest = async (responsibleIds: string[]) => {
    ...
    try {
      ...
      addToast("success", t("toast.assignSuccessTitle"), responsibleIds.length > 0 ? t("toast.assignSuccessAssigned") : t("toast.assignSuccessRemoved"));
    } catch (error) {
      addToast("error", t("toast.assignErrorTitle"), getErrorMessage(error, t("toast.assignErrorDefault")));
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      addToast("success", t("toast.deleteSuccessTitle"), t("toast.deleteSuccess"));
      setConfirmModal({ isOpen: false, request: null });
    } catch (error) {
      addToast("error", t("toast.deleteErrorTitle"), getErrorMessage(error, t("toast.deleteErrorDefault")));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleStatusUpdate = async (requestId: string, newStatus: string) => {
    try {
      ...
      addToast("success", t("toast.statusUpdateSuccessTitle"), t("toast.statusUpdateSuccess"));
      ...
      addToast("error", t("toast.statusUpdateErrorTitle"), getErrorMessage(error, t("toast.statusUpdateErrorDefault")));
      throw error;
    }
  };
  ...
  const handleTogglePublic = async (id: string, newState: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        addToast('success', t("toast.publicOfferSuccessTitle"), newState ? t("toast.publicOfferEnabled") : t("toast.publicOfferDisabled"));
        ...
        addToast('error', t("toast.publicToggleErrorTitle"), t("toast.publicToggleError"));
      }
    } catch (error) {
      // Annuler l'état optimiste
      setOptimisticPublicStates(prev => {
  ...
  const columns = [
      ...
      render: (value: unknown, row?: ApplicationRequest) => {
        return (
          <div>
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400">{t("list.columns.reference", { ref: row?.reference || "-" })}</div>
          </div>
        );
      },
    },
    ...
    },
    {
      ...
      header: t("list.columns.publication"),
  ...
  return (
    <div>
        ...
        <div className="mb-5 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <input
                type="text"
                placeholder={t("list.filters.skillsPlaceholder")}
            ...
            <div>
              <select
                ...
                className="h-11 w-full appearance-none rounded-lg border border-gray-300 px-4 py-2.5 text-sm shadow-theme-xs focus:outline-hidden focus:ring-3 focus:border-brand-300 focus:ring-brand-500/10 dark:bg-gray-900 dark:text-white/90 dark:border-gray-700 dark:focus:border-brand-800"
              >
                <option value="">{t("list.filters.allExperienceLevels")}</option>
            ...
            <div>
              <select
                ...
                className="h-11 w-full appearance-none rounded-lg border border-gray-300 px-4 py-2.5 text-sm shadow-theme-xs focus:outline-hidden focus:ring-3 focus:border-brand-300 focus:ring-brand-500/10 dark:bg-gray-900 dark:text-white/90 dark:border-gray-700 dark:focus:border-brand-800"
              >
                <option value="">{t("list.filters.allContractTypes")}</option>
      ...
      <ConfirmModal
        ...
        message={t("confirmModal.message", { title: confirmModal.request?.title || "" })}
...
function KanbanIcon() {
  return <LayoutDashboard className="icon-glow" size={16} strokeWidth={1.8} />;
}
