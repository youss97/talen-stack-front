...
} from "@/lib/services/crmIndustryApi";
...
export default function CrmIndustriesPage() {
  const t = useTranslations("settings.crmIndustries");
  const tc = useTranslations("common");
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useLimitPreference("settings-crm-industries", 20);
  const [search, setSearch] = useState("");
  const [isFormModalOpen, setIsFormModalOpen] = useState(false);
  const [selectedIndustry, setSelectedIndustry] = useState<CrmIndustryRow | null>(null);
  const [toasts, setToasts] = useState<ToastItem[]>([]);
  const [confirmModal, setConfirmModal] = useState<{ isOpen: boolean; industry: CrmIndustryRow | null }>({
    isOpen: false,
    industry: null,
  });

  const { data, isLoading, refetch } = useGetCrmIndustriesQuery({ page, limit, search });
  const [createIndustry, { isLoading: isCreating }] = useCreateCrmIndustryMutation();
  const [updateIndustry, { isLoading: isUpdating }] = useUpdateCrmIndustryMutation();
  const [deleteIndustry, { isLoading: isDeleting }] = useDeleteCrmIndustryMutation();
  ...
  const handleOpenEdit = (industry: CrmIndustryRow) => {
    setSelectedIndustry(industry);
    setFormData({ name: industry.name, is_active: industry.is_active });
    setIsFormModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedIndustry) {
        await updateIndustry({ id: selectedIndustry.id, data: formData }).unwrap();
      ...
      addToast("error", tc("status.error"), getApiErrorMessage(error, t("toasts.genericError")));
    }
  };

  const handleDelete = async () => {
    if (!confirmModal.industry) return;
    try {
      await deleteIndustry(confirmModal.industry.id).unwrap();
      addToast("success", tc("status.success"), t("toasts.deleted"));
      setConfirmModal({ isOpen: false, industry: null });
      refetch();
    } catch (error) {
      addToast("error", tc("status.error"), getApiErrorMessage(error, t("toasts.deleteError")));
    }
  };

  const columns: Column<CrmIndustryRow>[] = [
    {
      key: "name",
  ...
  return (
    <>
      ...
      <DataTable
        ...
        onDelete={(row) => setConfirmModal({ isOpen: true, industry: row })}
      />
      ...
      <Modal isOpen={isFormModalOpen} onClose={() => setIsFormModalOpen(false)} className="max-w-md">
        <div className="p-6">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white/90 mb-4">
            {selectedIndustry ? t("modal.editTitle") : t("modal.createTitle")}
          </h2>
      ...
      <ConfirmModal
        ...
        onClose={() => setConfirmModal({ isOpen: false, industry: null })}
        onConfirm={handleDelete}
        title={t("deleteModal.title")}
        message={t("deleteModal.message", { name: confirmModal.industry?.name || "" })}
