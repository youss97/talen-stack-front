...
export default function SidebarOrderPage() {
  const t = useTranslations("settings.sidebarOrder");
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Button onClick={handleSave} disabled={items.length === 0}>{t("saveButton")}</Button>
      </div>
              ...
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button type="button" onClick={() => move(i, -1)} disabled={i === 0}
                    className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-40" aria-label={t("moveUp")}>↑</button>
                  <button type="button" onClick={() => move(i, 1)} disabled={i === items.length - 1}
                    className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-40" aria-label={t("moveDown")}>↓</button>
                </div>
              </li>
            ))}
          </ul>
